<template>
  <div>
      ...
      <v-tooltip bottom>
        <template v-slot:activator="{ on, attrs }">
          <v-btn :outlined="attrs['aria-expanded'] == 'true'" color="primary" v-on="on" v-bind="attrs" fab small dark
            elevation="1" @click="openFormulario">
    ...
    <v-data-table :items="lst_socios" :headers="headers" :search="search" :page.sync="page"
      ...
      <template v-slot:item.estatus="{ item }">
        <v-chip v-if="item.estatus == 1" color="success" label small class="font-weight-bold">Con Acceso
          Vigente</v-chip>
        <v-chip v-else color="error" label small>Sin Acceso Vigente</v-chip>
      </template>
      <template v-slot:item.ac="{ item }">
        <v-btn v-if="item.estatus == 0" color="info" @click="fnOpenReingreso(item)" small elevation="1" outlined rounded class="text-none">Reingresar{{ cve_accion }}</v-btn>
        <v-chip v-else small>{{ dayjs(item.fecha_fin).format('DD [de] MMM [de] YYYY') }}</v-chip>
      </template>

      <template v-slot:item.detalle="{ item }">
        <v-btn color="warning" small elevation="1" outlined rounded @click="CargarDetalle(item.id)"
          class="text-none"><v-icon class="mr-2">mdi-eye</v-icon>Ver Detalle</v-btn>
      </template>

      <template v-slot:item.historico="{ item }">
        <v-btn color="accent" @click="CargarHistorico(item.id)" small elevation="1" outlined rounded
          class="text-none"><v-icon class="mr-2">mdi-eye</v-icon>Ver Historico</v-btn>
      </template>
    </v-data-table>
    ...
    <v-dialog v-model="dialog" fullscreen scrollable content-class="overflow-y-hidden">
      <v-card rounded="0">
        <v-toolbar elevation="0" max-height="64">
          <v-toolbar-title class="secondary--text">Formulario para Alta de Invitado</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn @click="cerrarRegistro" icon color="secondary"><v-icon>mdi-close</v-icon></v-btn>
        </v-toolbar>
        <v-divider></v-divider>
        ...
        <v-card-text class="grey lighten-4">
          ...
          <v-row wrap>
            <v-col lg="3">
              <v-select label="Accion Invitado" v-model="cve_accion" filled :items="lst_acciones"
                item-value="cve_accion" item-text="numero_accion" @change="posicionesDisponibles"
                :error-messages="iErrors.cve_accion"></v-select>
            </v-col>
            <v-col lg="3">
              <v-text-field label="Nombre" v-model="nombre" filled class="my-mayus"
                :error-messages="iErrors.nombre"></v-text-field>
            </v-col>
            <v-col lg="3">

              <v-text-field label="Apellido Paterno" v-model="paterno" filled class="my-mayus"
                :error-messages="iErrors.paterno"></v-text-field>
            </v-col>
            <v-col lg="3">
              <v-text-field label="Apellido Materno" v-model="materno" filled class="my-mayus"
                :error-messages="iErrors.materno"></v-text-field>
            </v-col>

            <v-col lg="3">
              <v-radio-group row v-model="genero" :error-messages="iErrors.genero">
                <v-radio label="Masculino" color="primary" value="Masculino"></v-radio>
                <v-radio label="Femenino" color="primary" value="Femenino"></v-radio>
              </v-radio-group>
            </v-col>
              ...
              <v-menu v-model="menu5" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
                ...
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field filled
                    ...
                    label="Fecha Nacimiento" prepend-inner-icon="mdi-calendar" v-on="on" v-bind="attrs"
            ...
            <v-col lg="3">
              <v-select :items="lst_nacionalidad" item-value="cve_pais" item-text="nombre" v-model="nacionalidad"
                label="Nacionalidad" filled :error-messages="iErrors.nacionalidad"></v-select>
            </v-col>
            <v-col lg="3">
              <v-select filled label="Estado Civil" v-model="estado_civil" :items="['Casado', 'Soltero', 'Divorciado']"
                :error-messages="iErrors.estado_civil"></v-select>
            </v-col>

            <v-col lg="3">

              <v-text-field label="Correo" v-model="correo" prepend-inner-icon="mdi-at" filled
                :error-messages="iErrors.correo"></v-text-field>
            </v-col>


            <v-col lg="3">

              <v-text-field label="Celular" v-model="celular" prepend-inner-icon="mdi-cellphone-android" filled
                :error-messages="iErrors.celular"></v-text-field>
            </v-col>


            <v-col lg="3">
              <v-select label="Profesion" v-model="profesion" :items="lst_profesion" item-value="cve_profesion"
                item-text="nombre" filled :error-messages="iErrors.profesion"></v-select>
            </v-col>
            <!-- <v-col lg="1">
            ...
            <v-col lg="2">
              <v-text-field label="Grado de Estudios" v-model="grado_estudio" filled
                :error-messages="iErrors.grado_estudio"></v-text-field>
            </v-col>



            <v-col lg="2">
              <v-text-field v-mask="'#####'" @keyup="getColonias" maxlength="5" v-model="cp" label="Código postal"
                filled></v-text-field>
            </v-col>
            <v-col lg="3">
              <v-select label="Colonia" :items="lst_colonias" item-value="cve_colonia" item-text="nombre"
                v-model="colonia" filled :error-messages="iErrors.colonia"></v-select>
            </v-col>
            ...
            <v-col lg="1">
              ...
              <v-text-field v-model="num_ext" maxlength="6" label="Número exterior" filled
                :error-messages="iErrors.num_ext"></v-text-field>
            </v-col>
            <v-col lg="1">
              <v-text-field v-model="num_int" maxlength="6" label="Número interior" filled></v-text-field>
            </v-col>
          ...
          <div class="primaryxx d-flex align-center justify-space-between">
            <div class="d-flex align-center">
              <span class="mr-3 title" :class="{ 'text--decorate': !tipo_acceso }">Por Dias</span>
              <v-switch v-model.number="tipo_acceso" :true-value="1" :false-value="0" color="primary"></v-switch>
              <span class="ml-3 mr-10 title" :class="{ 'text--decorate': tipo_acceso }">Periodo</span>
            </div>

            <div class="d-flex align-center">
              <span class="mr-3 title" :class="{ 'text--decorate': !tipo_invitado }">Libre</span>
              <v-switch v-model.number="tipo_invitado" :true-value="1" :false-value="0" color="primary"
                @change="changeTipoInvitado"></v-switch>
              <span class="ml-3 title" :class="{ 'text--decorate': tipo_invitado }">Socio Invita</span>
            </div>
          </div>
          ...
          <v-row v-if="tipo_acceso == 1">
            <v-col cols="12" md="3">
              <v-menu v-model="menu1" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
                offset-y min-width="290px">
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field v-model="fecha_inicio" label="Fecha Inicio" prepend-inner-icon="mdi-calendar" readonly
                    v-bind="attrs" v-on="on" filled :error-messages="iErrors.fecha_inicio"></v-text-field>
                </template>
                <v-date-picker :max="fecha_fin" no-title v-model="fecha_inicio" @input="menu1 = false"></v-date-picker>
              </v-menu>
            </v-col>
            <v-col cols="12" md="3">
              <v-menu v-model="menu2" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
                offset-y min-width="290px">
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field v-model="fecha_fin" label="Fecha Fin" prepend-inner-icon="mdi-calendar" readonly
                    v-bind="attrs" v-on="on" filled :error-messages="iErrors.fecha_fin"></v-text-field>
                </template>
                <v-date-picker :min="fecha_inicio" no-title v-model="fecha_fin" @input="menu2 = false"></v-date-picker>
              </v-menu>
            </v-col>

            <v-col cols="12" md="3">
              <v-text-field v-model="n_accion" label="Accion" @change="buscarSociosAccion"
                @keyup.enter="buscarSociosAccion" v-mask="accionMask" :disabled="tipo_invitado === 0"
                filled></v-text-field>
            </v-col>

            <v-col cols="12" md="3">
              <v-select v-model="cve_socio_invita" label="Socio Invita" :items="lst_socios_accion"
                item-value="cve_socio" :item-text="(i) => {
            ...
            <v-col cols="12" md="3">
              <v-text-field label="Costo" v-mask="'####################'" v-model="total" filled
                :error-messages="iErrors.total"></v-text-field>
            </v-col>
          </v-row>

          <v-row v-else>
            <v-col>
              <v-date-picker landscape v-model="dias_acceso" multiple color="primary" class="elevation-5"
                :error-messages="iErrors.dias_acceso"></v-date-picker>
            </v-col>

            <v-col>
              <v-row>
                <v-col cols="12" md="12">
                  <v-text-field v-model="n_accion" label="Accion" @keyup.enter="buscarSociosAccion" v-mask="accionMask"
                    :disabled="tipo_invitado === 0"></v-text-field>
                </v-col>
                <v-col cols="12" md="12">
                  <v-select v-model="cve_socio_invita" label="Socio Invita" :items="lst_socios_accion"
                    item-value="cve_socio" :item-text="(i) => {
                ...
                <v-col cols="12" md="12">
                  <v-text-field v-mask="'####################'" label="Costo" v-model="total"
                    :error-messages="iErrors.total"></v-text-field>
                </v-col>
              </v-row>
            </v-col>
        ...
        <v-card-actions class="grey lighten-4">
          <v-row>
            <v-col class="text-right">
              <v-btn color="success" width="220" @click="Guardar"><v-icon class="mr-2">mdi-content-save</v-icon>
                Guardar</v-btn>
            </v-col>

            <v-col class="text-left">
              <v-btn color="error" width="220" @click="Limpiar"><v-icon class="mr-2">mdi-close</v-icon>Limpiar</v-btn>
            </v-col>
          </v-row>
        </v-card-actions>
      </v-card>
    ...
    <!-- dialogo reingreso -->
    <v-dialog v-model="dialogReingreso" width="500" persistent scrollable>
      <v-card rounded="xl">
        <v-toolbar elevation="0">
          <v-toolbar-title>Reingresar Invitacion
          </v-toolbar-title>
          <v-spacer></v-spacer>
            <v-btn icon @click="fnCancelarReingreso">
        ...
        <v-card-text class="grey lighten-4">
          <v-row>
            <v-col cols="6" class="my-0 py-0 pr-0">
              <div class="d-flex align-center">
                <span class="mr-3 titlexx" :class="{ 'text--decorate': !tipo_acceso }">Por Dias</span>
                <v-switch v-model.number="tipo_acceso" :true-value="1" :false-value="0" color="primary"></v-switch>
                <span class="ml-3 mr-10" :class="{ 'text--decorate': tipo_acceso }">Periodo</span>
              </div>
            </v-col>

            <v-col cols="6" class="my-0 py-0">
              <div class="d-flex align-center">
                <span class="mr-3y" :class="{ 'text--decorate': !tipo_invitado }">Libre</span>
                <v-switch v-model.number="tipo_invitado" :true-value="1" :false-value="0" color="primary"
                  @change="changeTipoInvitado"></v-switch>
                <span class="ml-3 titlex" :class="{ 'text--decorate': tipo_invitado }">Socio Invita</span>
              </div>
            </v-col>
            ...
            <v-col cols="12" class="my-0 py-0" v-if="tipo_acceso == 1">
              <v-menu v-model="menu3" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
                ...
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field v-model="fecha_inicio" label="Fecha Inicio" prepend-inner-icon="mdi-calendar" readonly
                    v-bind="attrs" v-on="on" filled :error-messages="iErrorsReingreso.fecha_inicio"></v-text-field>
                </template>
                <v-date-picker :max="fecha_fin" no-title v-model="fecha_inicio" @input="menu3 = false"></v-date-picker>
              </v-menu>
            </v-col>
            <v-col cols="12" class="my-0 py-0" v-if="tipo_acceso == 1">
              <v-menu v-model="menu4" :close-on-content-click="false" :nudge-right="40" transition="scale-transition"
                ...
                <template v-slot:activator="{ on, attrs }">
                  <v-text-field v-model="fecha_fin" label="Fecha Fin" prepend-inner-icon="mdi-calendar" readonly
                    v-bind="attrs" v-on="on" filled :error-messages="iErrorsReingreso.fecha_fin"></v-text-field>
                </template>
                <v-date-picker :min="fecha_inicio" no-title v-model="fecha_fin" @input="menu4 = false"></v-date-picker>
              </v-menu>
            </v-col>

            <v-col cols="12" md="3" class="my-0 py-0 mx-0" v-if="tipo_acceso == 0">
              <v-date-picker landscape v-model="dias_acceso" multiple color="primary"
                class="elevation-5"></v-date-picker>
            </v-col>

            <v-col cols="12" md="12">
              <v-text-field v-model="n_accion" label="Accion" @keyup.enter="buscarSociosAccion" v-mask="accionMask"
                :disabled="tipo_invitado === 0" filled></v-text-field>
            </v-col>
            ...
            <v-col cols="12" md="12">
              <v-text-field v-mask="'####################'" label="Costo" v-model="total" filled :error-messages="iErrorsReingreso.total"></v-text-field>
            </v-col>
        ...
        <v-divider></v-divider>
          <v-card-actions class="px-5 pb-4 pt-3">
            ...
            <v-btn block color="primary" @click="reingreso"><v-icon>mdi-content-save</v-icon>Guardar
              Invitacion</v-btn>
          </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <!-- dialogo historico -->
    <v-dialog persistent width="600" v-model="dialogHistorico" content-class="rounded-xl">
      <v-card rounded="xl">
        <v-toolbar elevation="0">

          <v-toolbar-title>LISTADO DE INVITACION
          </v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-card-text class="px-0 grey lighten-4">
          <v-list color="grey lighten-4">
            <template v-for="(historico, i) in lst_historico" >
            <v-list-item two-line >
              <v-list-item-content>
                <v-list-item-title class="d-flex justify-space-between">
                  <div>
                    <span>{{ historico.nombre }}</span>
                    <span class="font-weight-bold mx-1">{{ historico.apellido_paterno }}</span>
                    <span class="font-italic">{{ historico.apellido_materno }}</span>
                  </div>

                  <div><span class="text-subtitle-2 font-weight-bold accent--text">Folio:</span> <span
                      class="text-subtitle-2">{{ historico.folio }} </span></div>
                </v-list-item-title>
                <v-list-item-subtitle class="d-flex" style="column-gap: 25px;">
                  <div><b class="mr-1">Fecha registro:</b>{{ historico.fecha_registro }}</div>
                  <div><b class="mr-1">Periodo:</b>{{ historico.fecha_inicio }} a {{ historico.fecha_fin }}</div>
                </v-list-item-subtitle>
                <v-list-item-subtitle class="d-flex" style="column-gap: 25px;">

                  <div><b class="mr-1">Fecha cobro:</b>{{ historico.fecha_hora_cobro }} </div>
                  <div><b class="mr-1">Pago:</b>{{ numeral(historico.total).format("$0,0.00") }}</div>
                </v-list-item-subtitle>
              </v-list-item-content>
              <v-list-item-action>

                <div class="d-flex flex-column align-center justify-center">
                  <span style="font-size: 10px;">Accion</span>
            ...
            <v-divider inset v-if="i<lst_historico.length-1"></v-divider>
            </template>
          </v-list>
        </v-card-text>
      </v-card>
    ...
    <v-dialog v-model="dialogDetalle" persistent width="450" content-class="rounded-xl">
      <v-card rounded="xl">
        <v-toolbar color="primaryx" elevation="0">
          <v-toolbar-title class="secondary--text">Detalle Invitado</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-btn icon="" color="secondary" @click="dialogDetalle = false">
            <v-icon>mdi-close</v-icon>
          </v-btn>

        </v-toolbar>
        <v-divider></v-divider>
        <v-card-text class="pt-5 px-0 grey lighten-4">
          <v-list color="grey lighten-4" dense>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.nombre }} <span class="font-weight-bold">{{
                  detalleSocio.apellido_paterno }}</span> <span class="font-italic">{{ detalleSocio.apellido_materno
            ...
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.sexo }} </v-list-item-title>
                <v-list-item-subtitle>Genero</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ dayjs(detalleSocio?.fecha_nacimiento).format(`DD [de] MMMM [de] YYYY`) }} </v-list-item-title>
                <v-list-item-subtitle>Fecha Nacimiento</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.nacionalidad }} </v-list-item-title>
                <v-list-item-subtitle>Nacionalidad</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.estado_civil }} </v-list-item-title>
                <v-list-item-subtitle>Estado Civil</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.celular }} </v-list-item-title>
                <v-list-item-subtitle>Celular</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.correo_electronico }} </v-list-item-title>
                <v-list-item-subtitle>Correo</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1"> {{ detalleSocio?.calle }} {{ detalleSocio?.numero_exterior
                }}{{
                    detalleSocio?.numero_interior }} <span class="font-weight-bold accent--text mx-1">cp:</span>{{
                    detalleSocio?.cp }} {{ detalleSocio?.colonia }} </v-list-item-title>
                <v-list-item-subtitle>Direccion</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.grado_estudio }} </v-list-item-title>
                <v-list-item-subtitle>Grado Estudio</v-list-item-subtitle>
              </v-list-item-content>
            </v-list-item>
            <v-list-item>
              <v-list-item-content>
                <v-list-item-title class="text-subtitle-1">{{ detalleSocio?.profesion }} </v-list-item-title>
    ...
    <v-dialog v-model="dialogInvitadoSugerencia" persistent scrollable width="500">
      <v-card rounded="xl">
        <v-card-title class="d-flex justify-space-between primary white--text">Sugerencia de invitados
          <v-btn icon="" color="white" @click="dialogInvitadoSugerencia = false">
            <v-icon>mdi-close</v-icon>
          </v-btn></v-card-title>
        <v-card-text>
          <v-list three-line>
            <v-list-item cols="12" md="4" v-for="(sugerencia_invitado, index) in lst_invitados_sugerencia" :key="index"
...
import { useValidacionInvitado,useValidacionInvitadoReingreso } from '@/composables/useValidacionInvitados'



const root = getCurrentInstance().proxy;
const { iValid, iErrors, iClear } = useValidacionInvitado()
const { iValid:iValidReingreso, iErrors:iErrorsReingreso, iClear:iClearReingreso } = useValidacionInvitadoReingreso()
// #region variables
//datos del invitado
...
const Guardar = debounce(async () => {
  ...
  console.log("data valid", data_valid)
  ...
  const data = await createInvitadoService(data_valid)
...
const reingreso = debounce(async () => {
  ...
  console.log(dataSend)
  const data_valid=await iValidReingreso(dataSend)
  console.log(data_valid)
  
  const data = await reingresoInvitadoService(cve_invitado.value, data_valid)
...
function fnOpenReingreso(reingreso)
{
  console.log(reingreso)
  cve_accion.value = reingreso.cve_accion
  cve_invitado.value = reingreso.id
  cve_persona.value = reingreso.cve_person
  dialogReingreso.value = true
}
