...
import {getTipoAccionService,findAccionByParameterService} from '@/services/acciones_service'
    ...
    function detalle(item) {
      ...
      console.log("🚀 ~ file: view_catalogo_acciones_recepcion.vue:105 ~ detalle ~ item:", item);

      console.log(item.cve_accion)
      root.$ls.set("accion_recepcion",item)
      root.$router.push({ name: "detalleAccionRecepcion", params: { cve_accion: item.cve_accion } });
    }
