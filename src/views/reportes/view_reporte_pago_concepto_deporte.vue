<template>
  <div>
    <div class="d-flex justify-space-between align-center mb-5">
      <h1 class="accent--text oswald--text">Reporte de pagos deporte</h1>
      <div>
    ...
    <v-row>
      ...
      <v-col>
        ...
        >
          <template v-slot:activator="{ on }">
            <v-text-field
              :value="dayjs(date_fecha_inicio??null).isValid()?dayjs(date_fecha_inicio).format('D [de] MMMM [de] YYYY'):''"
              label="Dia Inicio"
      ...
      <v-col>
        ...
        >
          <template v-slot:activator="{ on }">
            <v-text-field
              :value="dayjs(date_fecha_fin??null).isValid()?dayjs(date_fecha_fin).format('D [de] MMMM [de] YYYY'):''"
              label="Dia Fin"
              prepend-inner-icon="mdi-calendar"
              readonly
              v-on="on"
...
import {getCajeroService,getConceptosDeportesService,getBuscarPagosDeportesService,getPagosExportarService} from '@/services/reporte_pagos_service'
...
const cValid = computed(() =>(!Boolean(accion.value) && !Boolean(concepto.value) && !Boolean(date_fecha_inicio.value) && !Boolean(date_fecha_fin.value)));
...
async function getConceptos() {
  const data=await getConceptosDeportesService();
...
async function buscar() {
  ...
  if (Array.isArray(tripartida)) {
    numero_accion = accion.value.substring(0, tripartida.index);    
    clasificacion = tripartida.reduce((reductor, value) => reductor[value], {A: 1,B: 2,C: 3})    
  } else {
    numero_accion = Boolean(accion.value) ?accion.value:undefined;
    clasificacion = Boolean(accion.value) ?0:undefined;
  }
  const {conceptos}=await getBuscarPagosDeportesService(numero_accion,clasificacion,concepto.value,fecha_inicio,fecha_fin)
...
function generar() {
  ...
  let cajero_selected = lst_cajero.value.find(
    (item) => item.cve_persona == 1//cajero.value
  );
