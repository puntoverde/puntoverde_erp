<template>
  <div id="view-reporte-print">
    <div class="d-flex justify-space-between">
      ...
      <h1 class="accent--text oswald--text">Revision de Incidencias Final</h1>
      <div class="d-flex" style="column-gap: 5px;">
        ...
        <v-btn fab dark small color="blue" @click="fnAsistenciaFullColaboradores">
            ...
            <tr :key="ind" v-if="dia?.descanso">
              ...
              <template v-if="dia?.asistencia">
                ...
                <td class="px-0" style="background-color: #fffde7">
                  ...
                  <span v-if="asistencia_entrada == 'Omisión'" class="font-italic">{{ asistencia_entrada }}
                  </span>

                  <span v-else-if="asistencia_entrada == '--'" class="font-italic">
                  </span>

                  
                    
                          <span  class="font-weight-bold accent--text" >{{asistencia_entrada?.relog}} </span>
                ...
                <td  class="px-0" style="background-color: #fffde7">
                  ...
                  <span v-if="asistencia_salida == 'Omisión'" class="font-italic">{{ asistencia_salida }}</span>

                  <span v-else-if="asistencia_salida == '--'" class="font-italic">
                  </span>

                  
                  <span  class="font-weight-bold accent--text" >{{asistencia_salida?.relog}} </span>
...
<v-card-text class="mt-8">
  <div class="pv_time_ text-h1 d-flex justify-center my-5">
    <input id="input-time" v-model="time_revision_horas" placeholder="00:00"style="width: 100%; text-align: center" v-mask="timeMask"/>
  </div>
...
import {
  ...
  getAsistenciaFullColaboradoresService
    ...
    lst_datos.value = data.map((i) => {
      ...
      if (descanso && _asistencia?.length > 0) {
        ...
        if (Boolean(asistencia_salida)) {
          ...
          return { ...h_default, asistencia: _asistencia, asistencia_entrada:[asistencia_entrada], asistencia_salida: [asistencia_salida], rango_horas,extras_ };
        }
        else {
          return { ...h_default, asistencia: _asistencia, asistencia_entrada:[asistencia_entrada], asistencia_salida: ['Omisión'], rango_horas: 'Omisión', extras_};
        }
      }
...
function print() {
  /**table cabecera */
  const table = $dom("table", {style: { width: "100%", borderCollapse: "collapse", marginBottom: "10px" }});
  const table_body = $dom("tbody");
  const table_body_row1 = $dom("tr", [
    $dom("td", "No. empleado", {style: { width:"80px",border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", nomina.value, {style: {width:"70px", border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("td", "Nombre", {style: {width:"50px", border: "1px solid #000", fontSize: "12px",fontWeight:'bold'}}),
    $dom("td", nombre_colaborador.value, {style: { border: "1px solid #000", fontSize: "12px" }}),

    $dom("td", "Retardo menor", {style: {width:"85px", border: "1px solid #000", fontSize: "12px" ,fontWeight:'bold'}}),
    $dom("td", cRMenor.value, {style: { border: "1px solid #000", fontSize: "12px" }}),

    $dom("td", "Faltas", {style: {width:"85px", border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", cFaltas.value, {style: { border: "1px solid #000", fontSize: "12px" }}),
  ]);
  const table_body_row2 = $dom("tr", [
    $dom("td", "Departamento", {style: { border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", departamento.value, {style: { border: "1px solid #000", fontSize: "12px" ,textAlign:'center'}}),
    $dom("td", "Periodo", {style: { border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", `${dayjs(date.value[0]).locale("es").format("DD [de] MMMM")} al ${dayjs(date.value[1]).locale("es").format("DD [de] MMMM")}`,{ style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", "Retardo mayor", {style: { border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", cRMayor.value, {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", "Permisos", {style: { border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", cPermiso.value, {style: { border: "1px solid #000", fontSize: "12px" }})
    
  ]);
  const table_body_row3 = $dom("tr", [
    $dom("td", "Total horas", {style: { border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
    $dom("td", numeral(cTotalHOras.value).format("0:0"), {style: { border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("td", { style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", { style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", "Vacaciones", {style: { border: "1px solid #000", fontSize: "12px",fontWeight:'bold' }}),
  ...
  const table2 = $dom("table", {style: { width: "100%", borderCollapse: "collapse", marginBottom: "10px" }});
  const table2_head = $dom("thead",[
    $dom("tr",[
    $dom("th", "Días", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Horario", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Hora entrada", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center'}}),
    $dom("th", "Hora salida", {style: { border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Extras", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Tiempo", {style: { border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Descripción", {style: { border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Estatus", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    // $dom("th", "", {style: { border: "1px solid #000", fontSize: "12px" }}),
    ])

  ]);
  const table2_body = $dom("tbody");


  const table2_tbody_trs=lst_datos.value.map(i=>{

    const horario_=i.horarios_full?.map(j=>$dom("div",j,{style:{fontSize:'10px'}}))
    const entrada_=(i?.asistencia_entrada?.map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[]
    // const entrada_=([].map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[]
    const salida_=(i?.asistencia_salida?.map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[]
    // const salida_=([].map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[]
    const extras_=[$dom("div",i?.extras_?.[0]?.relog,{style:{fontSize:'10px'}}),$dom("div",i?.extras_?.[1]?.relog,{style:{fontSize:'10px'}})]
    const descripcion=i?.descripcion?.join('')
    let estatus=''
    ...
    switch(i.estatus)
    {
      case 1: estatus="asistencia"; break;
      case 2: estatus="tolerancia"; break;
      case 3: estatus="retardo menor"; break;
      case 4: estatus="retardo mayor"; break;
      case 5: estatus="omision"; break;
      case 6: estatus="permiso"; break;
      case 7: estatus="vacaciones"; break;
      case 8: estatus="asueto"; break;
      case 9: estatus="incidencia"; break;
      case 10: estatus=""; break;
      default: estatus="falta";
    }
    ...
    if(i?.descanso)
    {
      if(i?.asistencia){
        return $dom("tr",[
    $dom("td", dayjs(i.dia).format("ddd DD MMM"), {style: {width:'70px',paddingLeft:'2px', border: "1px solid #000", fontSize: "10px" }}),
    $dom("td", ["descanso"], {style: {width:'80px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", [...entrada_], {style: {width:'80px',border: "1px solid #000", fontSize: "10px",textAlign:'center'}}),
    $dom("td", [...salida_], {style: { width:'80px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", extras_, {style: { width:'60px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", i?.rango_horas, {width:'60px',style: { border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("th", descripcion, {style: { border: "1px solid #000", fontSize: "9px",fontWeight:'normal' }}),
    $dom("td", estatus, {style: {width:'70px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    ])
      }
      else{
        return $dom("tr",[$dom('td',dayjs(i.dia).format("ddd DD MMM"),{style: {width:'70px',paddingLeft:'2px', border: "1px solid #000", fontSize: "10px" }}),$dom('td',"descanso",{colSpan:7,style: {width:'70px',paddingLeft:'2px', border: "1px solid #000",textAlign:"center", fontSize: "10px" }})])
      }
    }
    else{
    return $dom("tr",[
    $dom("td", dayjs(i.dia).format("ddd DD MMM"), {style: {width:'70px',paddingLeft:'2px', border: "1px solid #000", fontSize: "10px" }}),
    $dom("td", [...horario_], {style: {width:'80px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", [...entrada_], {style: {width:'80px',border: "1px solid #000", fontSize: "10px",textAlign:'center'}}),
    $dom("td", [...salida_], {style: { width:'80px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", extras_, {style: { width:'60px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", i?.rango_horas, {width:'60px',style: { border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
  ...
  $setChildren(table2_body, [...table2_tbody_trs]);

  $setChildren(table2, [table2_head,table2_body]);

  console.log(table2.cloneNode(true))

  const divPrint = $dom("div", { style: { fontFamily: "Arial"} }, [
    $dom("div",{style:{border:'0px solid #000',height:"135mm" }},[table,
    table2,]),
    $dom("div",{style:{border:'0px solid #000',height:"135mm" }},[table.cloneNode(true),
    table2.cloneNode(true),])
  ]);
...
//
function formatAsistenciasFull(data) {
  ...
  try {
    ...
    return  data.map((i) => {
      ...
      //asistencia reloj es la original ejemplo leon solo registro entrada 16:15:00 pero viene id|hora => {id,relog}
      const _asistencia_reloj = (i.asistencia ?? "").split(",").filter(Boolean)
      //asistencia revision o manual  leon solo registro entrada 16:15:00 pero no salida se agrega la salida manualmente y esta viene como id|id_|hora el id_ es el manual y id es el del relog o 0 si no existe pasa a =>{id,id_relog}
      const _asistencia_revision = (i.asistencia_revision ?? "").split(",").filter(Boolean)
      //es extras de forma manual y son igual que revision solo que se identifican con el 1 o 0 y son id|id_|hora => {id,id_,hora}
      const _extras = (i.extras ?? "").split(",").filter(Boolean)

      //es un array con los revisados como base y los de reloj quitando los de reloj que estan relaccinados con los revisados
      const _asistencia=[
        //todo los horarios revisados
        ..._asistencia_revision,
        //todos los horarios de reloj pero que no esten en lazados con algun horario revisado
        ..._asistencia_reloj]
        //se ordenan 
        .sort((a,b)=>dayjs(a,'HH:mm:ss').isAfter(dayjs(b,'HH:mm:ss'),'m')?1:-1)
      ...
      //2024-10-16 checadas
      //primera checada
      const FCHECK = first(_asistencia)
      const FCHECKDATE = dayjs(FCHECK, "HH:mm:ss")
...
function printFull(data_) {

  /**table cabecera */
  const table = $dom("table", {style: { width: "100%", borderCollapse: "collapse", marginBottom: "10px" }});
  const table_body = $dom("tbody");
  const table_body_row1 = $dom("tr", [
    $dom("td", "No. empleado", {style: { width:"80px",border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.nomina, {style: {width:"70px", border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("td", "Nombre", {style: {width:"50px", border: "1px solid #000", fontSize: "12px",}}),
    $dom("td", `${data_?.nombre} ${data_.apellido_paterno} ${data_.apellido_materno}`, {style: { border: "1px solid #000", fontSize: "12px" }}),

    $dom("td", "Retardo menor", {style: {width:"85px", border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.retardo_menor, {style: { border: "1px solid #000", fontSize: "12px" }}),

    $dom("td", "Faltas", {style: {width:"85px", border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.faltas, {style: { border: "1px solid #000", fontSize: "12px" }}),
  ]);
  const table_body_row2 = $dom("tr", [
    $dom("td", "Departamento", {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.departamento, {style: { border: "1px solid #000", fontSize: "12px" ,textAlign:'center'}}),
    $dom("td", "Periodo", {style: { border: "1px solid #000", fontSize: "12px" }}),
    // $dom("td", `${dayjs(date.value[0]).locale("es").format("DD [de] MMMM")} al ${dayjs(date.value[1]).locale("es").format("DD [de] MMMM")}`,{ style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", `${dayjs(date.value[0]).locale("es").format("DD [de] MMMM")} al ${dayjs(date.value[1]).locale("es").format("DD [de] MMMM")}`,{ style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", "Retardo mayor", {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.retardo_mayor, {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", "Permisos", {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.permisos, {style: { border: "1px solid #000", fontSize: "12px" }})    
  ]);
  const table_body_row3 = $dom("tr", [
    $dom("td", "Total horas", {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", numeral(data_?.total_horas).format("0:0"), {style: { border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("td", { style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", { style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", "Vacaciones", {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", data_?.vacaciones, { style: { border: "1px solid #000", fontSize: "12px" } }),
    $dom("td", {style: { border: "1px solid #000", fontSize: "12px" }}),
    $dom("td", {style: { border: "1px solid #000", fontSize: "12px" }})
  ...
  const table2_head = $dom("thead",[
    $dom("tr",[
    $dom("th", "Dias", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Horario", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Hora entrada", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center'}}),
    $dom("th", "Hora salida", {style: { border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
    $dom("th", "Extras", {style: {border: "1px solid #000", fontSize: "12px",textAlign:'center' }}),
  ...
  const table2_tbody_trs=data_.accesos?.map(i=>{

 
  // console.log('asistencia_entrada:',i.dia,data_.nomina,i?.asistencia_entrada,(i?.asistencia_entrada?.map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[])

    const horario_=i?.horarios_full?.map(j=>$dom("div",j,{style:{fontSize:'10px'}}))
    const entrada_=(i?.asistencia_entrada?.map(j=>$dom("div",j,{style:{fontSize:'10px'}})))??[]
    // const entrada_=([].map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[]
    const salida_=(i?.asistencia_salida?.map(j=>$dom("div",j,{style:{fontSize:'10px'}})))??[]
    // const salida_=([].map(j=>$dom("div",j?.relog,{style:{fontSize:'10px'}})))??[]
    const extras_=[$dom("div",i?.extras_?.[0]?.relog,{style:{fontSize:'10px'}}),$dom("div",i?.extras_?.[1]?.relog,{style:{fontSize:'10px'}})]
    const descripcion=i?.descripcion?.join('')
    let estatus=''
    ...
    // $dom("td", extras_, {style: { width:'60px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    // $dom("td", i?.rango_horas, {width:'60px',style: { border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    // $dom("th", descripcion, {style: { border: "1px solid #000", fontSize: "12px" }}),
    // $dom("td", estatus, {style: {width:'70px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    // ])
...
return  $dom("tr",[
    ...
    $dom("td", extras_, {style: { width:'60px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", i?.rango_horas, {width:'60px',style: { border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    $dom("td", descripcion, {style: { border: "1px solid #000", fontSize: "10px" }}),
    $dom("td", estatus, {style: {width:'70px',border: "1px solid #000", fontSize: "10px",textAlign:'center' }}),
    ])
    }
  ...
  const divPrint = $dom("div", { style: { fontFamily: "Arial",breakAfter:'page' } }, [
  $dom("div",{style:{border:'0px solid #000',height:"135mm" }},[table,
  table2,]),
  $dom("div",{style:{border:'0px solid #000',height:"135mm" }},[table.cloneNode(true),
    table2.cloneNode(true)])
...
function findMap(data,id_colaborador)
{
  return data.filter(i=>i.id_colaborador==id_colaborador)
    .map(i=>({
      asistencia:i.asistencia,
      asistencia_revision:i.asistencia_revision,
      descripcion:i.descripcion,
      dia:i.dia,
      dia_entrada:i.dia_entrada,
      dia_salida:i.dia_salida,
      extras:i.extras,
      hora_entrada:i.hora_entrada,
      hora_salida:i.hora_salida,
      is_asueto:i.is_asueto,
      is_incidencia:i.is_incidencia,
      is_permiso:i.is_permiso,
      is_vacaciones:i.is_vacaciones
    }))
}

async function fnAsistenciaFullColaboradores()
{
  ...
  //9->incidencia
  //10->dia que aun no es valido dia a futuro 
  const data=await getAsistenciaFullColaboradoresService(date.value)

  


  const data_fomrated=_uniqBy(data, 'id_colaborador').map(({
    id_colaborador,
    nomina,
    nombre,
    apellido_paterno,
    ...
    apellido_materno,
    departamento,
    retardo_menor:formatAsistenciasFull(findMap(data,id_colaborador)).filter(i=>i.estatus===3).length,
    retardo_mayor:formatAsistenciasFull(findMap(data,id_colaborador)).filter(i=>i.estatus===4).length,
    faltas:formatAsistenciasFull(findMap(data,id_colaborador)).filter(i=>i.estatus===0).length,
    vacaciones:formatAsistenciasFull(findMap(data,id_colaborador)).filter(i=>i.estatus===7).length,
    permisos:formatAsistenciasFull(findMap(data,id_colaborador)).filter(i=>i.estatus===6).length,
    total_horas:formatAsistenciasFull(findMap(data,id_colaborador)).filter((i) => !isUndefined(i?.rango_horas))
    .map((i) => i?.rango_horas.split(":"))
    .reduce((red, [h, m, s]) => {
      let horas = red.add(numeral(h).value(), "h");
      let minutos = horas.add(numeral(m).value(), "m");
    ...
    .diff(dayjs().startOf("year"), "s"),
    // accesos:formatAsistenciasFull(findMap(data,id_colaborador))
    accesos:formatAsistenciasFull(findMap(data,id_colaborador))
  }));

  console.log(data_fomrated)

  const div_print=data_fomrated.map(i=>{
    // printFull(i)
    return printFull(i)

  })

  // console.log(div_print)

  const div_print_=$dom('div',[...div_print])

printJS({ printable: div_print_.outerHTML, type: "raw-html",showModal:true,modalMessage:'Generando impresion',onLoadingStart:()=>{console.log('inicia carga de documento')} });


  //pasar por el filter map
