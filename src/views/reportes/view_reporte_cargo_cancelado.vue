<template>
  <div>
    <div class="d-flex justify-space-between">
      ...
      <h1 class="accent--text oswald--text">Cargos Cancelados</h1>
    </div>
...
import { getCargosReporteService } from "@/services/cargo_service";
...
const fechaInicioFormat = computed(() =>
  dayjs(fechaInicio.value).format("DD/MM/YYYY")
);
const fechaFinFormat = computed(() =>
  dayjs(fechaFin.value).format("DD/MM/YYYY")
);
...
const headersTabla = [
  ...
  { text: "Motivo", value: "motivo_cancelacion" },
...
async function llenarTabla(options = {}) {
  ...
  //  let {data, status}=await $axios.get("/cargo/reporte",{params:options})
  const data = await getCargosReporteService(options);
  // if (status===200){
