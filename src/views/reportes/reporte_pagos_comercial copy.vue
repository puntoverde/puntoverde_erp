          ...
          <v-expansion-panel-header @click="panel=item.idpago">
            ...
            <div class="" style="width: 90%">
              <div class="infox d-flex justify-space-between pr-10 primar my-3">
                ...
                >
                <label class="font-weight-black h1"
                  >Total: <v-chip outlined color="amber" label style="height: 25px;"><span class="accent--text">{{item.total | fcurrency}}</span></v-chip></label
                >
                <label class="font-weight-black h1"
                  >Subtotal: <span class="accent--text">{{item.subtotal | fcurrency}}</span></label
                >
                <label class="font-weight-black h1"
                  >Descuento: <span class="accent--text">{{item.descuento | fcurrency}}</span></label
                >
              </div>
              <div
                ...
                <template>
                  ...
                  <tbody>
                    <tr v-for="(cargo,index) in lst_cargos" :key="index">
                      ...
                      <td>{{cargo.descuento | fcurrency}}</td>
                      <td>
...
import { ref, defineComponent, onMounted, computed,set } from "@vue/composition-api";
import formAltaRfc from './form_repote_pagos_comercial'
import facturaSegmentada from './form_repote_pagos_comercial_segmentado'
import facturaDividida from './form_repote_pagos_comercial_dividido'
import {mask} from 'vue-the-mask'

export default defineComponent({
  components:{formAltaRfc,facturaSegmentada,facturaDividida},
  directives:{mask},
  setup(props, context) {
   const $axios =context.root.$axiosFinanzas;
   const $axiosCompac=context.root.$axiosCompaq;
...
if(status===200){
    ...
    lst_factura.value=factura
      ...
      function openCancelar(id){
        ...
        id_usuario_cancela.value=context.root.$ls.get('user',{cve_persona:0}).cve_persona
      }
      
      //#endregion
      
   return {lst_pagos,lst_cargos,getCargosAndFactura,panel,numeral,
   menu_date,menu_date2,fecha_inicio,fecha_fin,getPagos,
   n_accion,find_uuid,accionMask,
   itemsPerPage,itemsPerPageArray,page,numberOfPages,formerPage,nextPage,
   updateItemsPerPage,loading_pagos,facturaOk,
   saveFacturaPublicoGeneral,saldar,text_loading,lst_factura,
   saveFacturaPublicoGeneralCompac,saveFacturaCompac,snackbar,dialog_cancelar,EliminarFactura,
   motivo_cancelado,openCancelar,idfactura_cancelar,id_usuario_cancela,
   dayjs}
