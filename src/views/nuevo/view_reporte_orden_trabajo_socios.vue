    ...
    <div>
        <v-card class="mb-5">
            <v-toolbar elevation="0">
                <v-toolbar-title>Reporte orden trabajo usuarios</v-toolbar-title>
            </v-toolbar>
            <v-divider></v-divider>
...
<script setup>
import {ref} from 'vue'
import {getReporteSociosOrdenTrabajoService} from '@/services/orden_trabajo_service'
...
async function getOrdenTrabajoReporteSocios()
{
    ...
    try {
        lst_ordenes_trabajo.value=await getReporteSociosOrdenTrabajoService(folio.value,estatus.value)
    } catch (error) {
