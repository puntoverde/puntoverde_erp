<template>
  <div class="mt-5">
    <div class="d-flex justify-space-between mb-5">
      ...
      <h1 class="accent--text oswald--text">Validacion Nuevo Ingreso xxx</h1>
    ...
    <v-data-table :headers="headers" :items="items" :page.sync="page" :items-per-page="itemsPerPage"
      ...
      <template v-slot:item.estatus="{ item }">       
        <v-icon v-if="item.fecha_aceptacion" color="success">mdi-check-circle</v-icon>
    ...
    <!-- <img id="myImg" src="@/assets/image/img2.jpg" />
    <img id="myImg2" :src="img12" />
...
onMounted(async () => {
  ...
  const models_=await faceapi.nets.ssdMobilenetv1.loadFromUri('/models')
  const input_ = document.getElementById('myImg')

  // const detection = await faceapi.detectSingleFace(input_)
  const input_2= new Image()
  input_2.src=img1
  const detection = await faceapi.detectSingleFace(input_2)
  console.log("detection:",detection._box)

  resizeImage(img1,(detection._box._width+200),(detection._box._height+300),(detection._box._x-100),(detection._box._y-200),function (dataURL) {
