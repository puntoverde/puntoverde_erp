<template>
    <div>
        <div class="d-flex justify-space-between">
            ...
            <h1 class="accent--text oswald--text">Historial de Percanses</h1>
        </div>
            ...
            <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                    <v-btn :outlined="attrs['aria-expanded'] == 'true'" color="secondary" v-on="on" v-bind="attrs" fab small 
                        dark elevation="1" @click="openCreatePercance" class="mr-1">
        ...
        <v-data-table :headers="headers" :items="lst_percances" :search="search" :page.sync="page" :items-per-page="itemsPerPage"
            ...
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="secondary" @click="dialog_create=false">
                        ...
                        cancelar</v-btn>
                    <v-btn color="primary" @click="fnRegistrarPercance">
...
import { allPersonasService, findPersonaByIdService,registrarPercanceService,actualizarPercanceService,getPercancesService} from '@/services/historial_medico_service'
...
const headers = ref([
    ...
    { text: "Descripcion de los sucedido", align: "right", value: "descripcion" },
...
try {
  ...
  const data2 = await getPercancesService(id);
...
//datos de la persona salen de data
  ...
  lst_percances.value=data2
  console.log(lst_percances.value)
...
async function fnRegistrarPercance()
{
    try {
        ...
        const data=await registrarPercanceService(dataSend)
        console.log(data)

        const data2 = await getPercancesService(id_persona);
        lst_percances.value=data2
    } catch (error) {
