  ...
  >
    ...
    <template v-slot:activator="{ on }">
      <v-btn v-on="on" rounded dark smallx class="mx-1" color="primary">
        <v-icon color="white" class="mr-2">mdi-account-group</v-icon> SEGMENTADA
      </v-btn>
    </template>
    ...
    <v-card tile>
      <v-toolbar color="secondary" flat dark>
        <v-toolbar-title
          >Factura Segmentada con folio:{{foliopago}}</v-toolbar-title
        >
        <v-spacer></v-spacer>
        ...
        <v-tabs-items v-model="tab">
          <v-tab-item>
            <v-list subheaderxx three-line flat nav>
              <v-list-item-group v-model="factura" multiple>
                <v-list-item
                ...
                >
                  <template v-slot:default="{ active }">
                    ...
                    <v-list-item-content class="py-1">
                      <v-list-item-title><span class="text-subtitle-1">RFC:</span><span class="font-weight-bold">{{datos_factura.rfc}}</span></v-list-item-title>
                      <v-list-item-subtitle>
                        <span class="text-subtitle-2"> R-social:</span> <span class="font-weight-bold">{{datos_factura.razon_social}}</span>
                        </v-list-item-subtitle>
                        <v-list-item-subtitle>
                          <span class="text-subtitle-2"> Correo:</span>  <span class="font-weight-bold">{{datos_factura.correo}}</span>
                        </v-list-item-subtitle>
                      <v-list-item-subtitle>
                        <span class="text-subtitle-2">Domicilio:</span>
                        {{datos_factura.calle}} # {{datos_factura.num_ext}}, {{datos_factura.colonia}} cp:<span class="font-weight-bold">{{datos_factura.cp}}</span> {{datos_factura.municipio}} {{datos_factura.estado}}
                      </v-list-item-subtitle>
                    </v-list-item-content>
                  </template>
                </v-list-item>
                  ...
                  <template v-slot:default>
                    <thead class="secondary">
                      <tr>
                        ...
                        <th class="white--text text-center font-weight-bold text-body-1">
                          Periodo
                        </th>
                        <th class="white--text text-center font-weight-bold text-body-1">
                          Precio
                    ...
                    <tfoot>
                      <tr>
                      <th colspan="3" class="secondary" style="height:3px"></th>
                      </tr>                      
                    </tfoot>
                  </template>
                </v-simple-table> 
      ...
      <v-card-actions class="secondary">
        ...
        <v-spacer></v-spacer>
        <v-btn v-if="tab == 0 && CFacturas" @click="tab = 1" color="primary" outlined>Continuar</v-btn>
        <v-btn
...
const emit=defineEmits(["emit-facturado"])
    ...
    const CFacturas = computed(() => factura.value.length > 1);
    ...
    const PrecioValid = computed(() =>
      CFacturasSelection.value
        .map((i) => i.movimientos)//crea un array solo con los cargos 
        .flat()//realiza un map para tenerlos todo al mismo nivel
        .map((i) => numeral(i.importe).value())//crea un array solo los precios
        .every((i) => i > 0)//comprueba que todos los precios sean mayor a 0
    );
    
    //factura es el array llenado por item-group 
    watch(factura, () => {

console.log(dayjs(fecha_pago.value).format("DD/MM/YYYY"))
      const observaciones = `folio de pago:${foliopago.value} de la accion(${
        numero_accion.value
      ...
      CFacturasSelection.value = factura.value.map((i) => ({
        ...
        fecha_pago:dayjs(fecha_pago.value).format("DD/MM/YYYY"),
    ...
    const saveFacturaSegmentada = throttle(async () => {
       ...
       console.log("enviar facturar:",CFacturasSelection.value)
