  ...
  <v-dialog v-model="dialog" fullscreen>
    <template v-slot:activator="{ on, attrs }">
      <v-btn v-bind="attrs" v-on="on"
        ><v-icon>mdi-folder-account</v-icon>Portafolio Digital</v-btn
      >
    </template>
    <v-card tile>
...
import {
  documentosSocioService,
  deleteDocumentosSocioService,
...
async function getDocumentos() {
  ...
  const data = documentosSocioService(props.cve_socio);
  data.forEach((i) => {
    i.loading = false;
...
async function cargarFile(e, doc) {
  ...
  const { data, documento: file } = await cargarDocumentoSocioService(
    props.cve_socio,
    undefined,
    element.dataset.documento,
    element.files[0]
  );
...
async function deleteDocumento(doc) {
  ...
  //   `socios/${props.cve_socio}/documento`,
  //   { params: { cve_documento: doc.cve_documento } }
  // );
  const status = await deleteDocumentosSocioService(
    props.cve_socio,
    doc.cve_documento
  );
  if (status === 204) {
...
async function downloadFile(documento, mime) {
  window.open(
    `${import.meta.env.VITE_APP_RUTA_API}/socios/${
      props.cve_socio
    }/documento-file?documento=${documento}`
  );
}
