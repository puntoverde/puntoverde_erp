    ...
    <v-hover v-slot="{ hover }">
      <v-menu>
        <template v-slot:activator="{ on }">
          <v-badge
            bordered
            :color="parentesco === 11 ? 'accent' : 'transparent'"
            overlap
            left
          >
...
import {addFotoAccionistaService,deleteFotoAccionistaService,getFotoAccionistaService} from '@/services/accionistas_service'
...
watch(() => props.foto_socio,() =>getFoto());
...
{
  try {
  const confirm=await root.$confirm('Desea realmente eliminar la foto del dueño, seleccione opción <b>"Aceptar"</b> en caso contrario la opción <b>"Cancelar"</b>',{title:'info',width:350,persistent:true})
  if(confirm)
  {
