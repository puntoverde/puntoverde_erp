        ...
        <v-dialog v-model="dialog" width="90%" persistent>
            <template v-slot:activator>
                <v-tooltip bottom>
                    <template v-slot:activator="{ on }">
                        ...
                        <v-btn color="secondary" v-on="on" @click="open" :loading="loading">Iniciar revision
                        </v-btn>
                    </template>
                    <span>Revisar</span>
                </v-tooltip>
            ...
            <v-card>
                <v-toolbar color="primary" elevation="0">
                    <v-toolbar-title>Revisar Requisicion</v-toolbar-title>
                    <v-spacer></v-spacer>
                    ...
                    <template v-slot:default>
                        ...
                        <tbody>
                            ...
                            <td>
                                ...
                                <td>
                                    <v-chip v-if="producto.estatus_revision == 1" color="success">
                                ...
                                <td style="width: 100px;">
                                    <v-menu max-width="350" v-if="producto.estatus == 1 && producto.estatus_revision == 1">
                                        <template v-slot:activator="{ on }">
                                            <v-btn v-on="on" iconx outlined color="error">
                ...
                <v-card-actions class="secondary">
                    <v-spacer></v-spacer>
                    <v-btn v-if="estatus_requisicion == 1" color="white" outlined @click="cancelarRequisicion"><v-icon
                            class="mr-1">mdi-close</v-icon> Cancelar Requisicion</v-btn>
                    <v-btn v-if="estatus_requisicion == 1" color="primary" @click="marcarRevisionRequisicion"><v-icon
...
<script setup>
import { ref } from 'vue'
import { getDetalleRevision, cancelarProductoRequisicionRevision, finalizarRequisicionRevision, CancelarRequisicion } from '@/services/requisicion_service'
...
async function open() {
    loading.value = true
    try {
        lst_productos.value = await getDetalleRevision(props.id_requisicion)
        dialog.value = true
    } catch (error) {
        console.log(error)
    }
...
async function cancelarProducto(id_producto_requisicion) {
    try {
        const data = await cancelarProductoRequisicionRevision(id_producto_requisicion)
        console.log(data)
        if (data > 0) {
...
async function marcarRevisionRequisicion() {
    try {
        const data = await finalizarRequisicionRevision(props.id_requisicion)
        if (data > 0) {
            dialog.value = false
            emit('emit-finaliza')
        }
